...
namespace RyujiEv3
{
	...
	RyujiEv3Engine::RyujiEv3Engine(PortData initData)
	{
		...
		if(!pcolorSensor->init(initData.colorSensor)){return;}
		if(!psonarSensor->init(initData.sonarSensor)){return;}
		if(!pgyroSensor->init(initData.gyroSensor)){return;}
		if(!ptouchSensor->init(initData.touchSensor)){return;}

		if(!pleftMotor->init(initData.leftMotor)){return;}
		if(!prightMotor->init(initData.rightMotor)){return;}
		if(!ptailMotor->init(initData.tailMotor)){return;}
		if(!pbluetooth->init()){return;}
