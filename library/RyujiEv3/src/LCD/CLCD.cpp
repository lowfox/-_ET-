...
CLCD::CLCD() { setFont(LCD_Font::MEDIUM); }

CLCD::~CLCD() { ev3_image_free(&m_image); }

LCD_Font CLCD::getFont() const { return m_font; }

bool CLCD::clear() {
  return fillRect(0, 0, LCD_WIDTH, LCD_HEIGHT, LCD_Color::WHITE);
}

bool CLCD::fillRect(int32 x, int32 y, int32 width, int32 height,
                    LCD_Color color) {
