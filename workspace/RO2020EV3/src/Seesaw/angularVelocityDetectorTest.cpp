...
bool angularVelocityDetectorTest::setOffsetTest() {
  ...
  CaseAngVelSetOffset caseAngVelSetOffset[7] = {
      {-501, false}, {-500, true}, {-1, true},  {0, true},
      {1, true},     {500, true},  {501, false}};

  for (int i = 0; i < 7; i++) {
    if (caseAngVelSetOffset[i].retValue !=
        i_angularVelocityDetector.setOffsetValue(
            caseAngVelSetOffset[i].offsetValue)) {
      // rog
      EV3_LOG(
          "TEST_ERR: anglarVelocityDetector.setOffsetTest... i=%d, offset=%d , "
          "retValue=%d\n",
          i, caseAngVelSetOffset[i].offsetValue,
          caseAngVelSetOffset[i].retValue);
      return false;
    }
  }
  return true;
...
bool angularVelocityDetectorTest::detectTest() {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (retDetect[i] != i_angularVelocityDetector.detect()) {
      EV3_LOG(
          "TEST_ERR: anglarVelocityDetector.detect... i=%d, offset=%d , "
          "detectValue=%d , retValue=%d\n",
