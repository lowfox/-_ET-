...
/*
    EV3_LOG("hosei mae kakudo = %d",RyujiEv3Engine::GetTailMotor()->getCounts());
    if(!i_tailPositioner.run(&m_initTail)){      
        return false;}
    dly_tsk(2000);
    if(!i_tailPositioner.run(&m_asobiNoTail)){return false;}
    EV3_LOG("hosei nasi go kakudo = %d",RyujiEv3Engine::GetTailMotor()->getCounts());
    EV3_LOG("delay...15sec");
    dly_tsk(15000);
*/
    //�V�т̕␳�v���O�����B
...
/*
    if(!i_tailPositioner.run(&m_asobiNoTail)){return false;}
    EV3_LOG("hosei go kakudo = %d",RyujiEv3Engine::GetTailMotor()->getCounts());
    while(1){}
*/
    ...
    if(!RyujiEv3Engine::GetLeftMotor()->stop(false)){return false;} 
    if(!RyujiEv3Engine::GetRightMotor()->stop(false)){return false;}
