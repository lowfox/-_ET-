...
bool distanceRunner::run(infoDistanceRun* info){
    ...
    if(info==NULL){
        ...
        return false;}
    if(info->pwm > 100 || info->pwm < -100){return false;}
    if(info->distance > 790.0f || info->distance < -790.0f || info->distance==0.0f){return false;}

    straightRunner i_straightRunner;
    
    m_startDistance = Steering::GetDistance();

    if(!i_straightRunner.run(info->pwm)){return false;}

    if(info->distance > 0){
        while((Steering::GetDistance() - m_startDistance) <= info->distance){
            //EV3_LOG("distance=%f, nowDistance=%f\n",info->distance,Steering::GetDistance());
        }
    }else{
        while((Steering::GetDistance() - m_startDistance) >= info->distance){
            //EV3_LOG("distance=%f, nowDistance=%f\n",info->distance,Steering::GetDistance());
