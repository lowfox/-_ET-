...
bool garageInit::run(void) {
  ...
  while (nowCounts < 75) {
    ...
    if (!i_getUp.run(&m_lineTraseTail1, m_runUpDistance1)) {
      return false;
    }
    if (!i_braker.run()) {
      return false;
    ...
    if (!i_distanceRunner.run(&m_safeRun1)) {
      return false;
    }
    nowCounts = RyujiEv3Engine::GetTailMotor()->getCounts();
  }
  ...
  if (!i_getUp.run(&m_lineTraseTail4, m_runUpDistance3)) {
    return false;
  }
  if (!i_braker.run()) {
    return false;
  ...
  if (!RyujiEv3Engine::GetRightMotor()->setPWM(m_lineReturnPwm)) {
    return false;
  }
  ...
  if (!m_garageInitlineTracer.run(m_lineReturnPwm, m_garageInitAngle)) {
    return false;
  }
