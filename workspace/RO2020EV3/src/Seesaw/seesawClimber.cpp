...
bool seesawClimber::run(void) {
   ...
   */
  if (!i_wheelClimber.run()) {
    return false;
  }

  if (!i_distanceRunner.run(&m_postRun)) {
    return false;
  }

  //�K�����グ
  if (!i_getUp.run(&m_bodyClimbTail3, m_runUpDistance)) {
    return false;
  }
  // if(!i_straightRunner.run(60)){return false;}
  if (!i_tailPositioner.run(&m_tailClimbTail)) {
    return false;
  }
  if (!i_distanceRunner.run(&m_tailClimbPostRun1)) {
    return false;
  }
  if (!i_distanceRunner.run(&m_tailClimbPostRun2)) {
    return false;
  }
