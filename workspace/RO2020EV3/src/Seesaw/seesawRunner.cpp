...
bool seesawRunner::run(direction dir){
    ...
    */
        ...
        if(!i_tailPositioner.run(&m_forwardBentTail)){return false;}
        if(!Drive::SetDriveMode(DriveMode::Nomal)){EV3_LOG("SetDriveMode...return false;");}
        ...
        //if(!i_braker.run()){return false;}
        //dly_tsk(1000);
        if(!i_distanceRunner.run(&m_preRunBack)){return false;}
        dly_tsk(1000);
        if(!i_angularVelocityDetector.setOffsetValue(m_backBentOffset)){return false;}
        if(!i_straightRunner.run(m_backPwm)){return false;}
        /*
        ...
        */
        ...
        if(!i_tailPositioner.run(&m_backBentTail)){return false;}
