...
bool seesawDesc::run(void) {
  ...
  if(i_angleDetector.detect()){
      ...
      {
          if(!i_angleDetector.setTargetValue(m_bodyDescTargetValue)){return
  false;} if(i_angleDetector.detect()){ m_initState=3;//車体がシーそー下にある
          }else{
              return false;
          }
      }
  ...
  */
  ...
  if (!i_distanceRunner.run(&m_preRun)) {
    return false;
  }
  if (!i_tailPositioner.run(&m_descTail)) {
    return false;
  }
  dly_tsk(600);
  RyujiEv3Engine::GetGyroSensor()->setOffset(0);
  if (!i_angularvelocityDetector.setOffsetValue(m_wheelDescOffset)) {
    return false;
  }

  if (!i_straightRunner.run(m_wheelDescPwm)) {
    return false;
  }
  ...
  if (!i_distanceRunner.run(&m_wheelAfterRun)) {
    return false;
  }
  if (!i_braker.run()) {
    return false;
  ...
  if (!i_tailPositioner.run(&m_tailDescTail)) {
    return false;
  }
  // if(!i_distanceRunner.run(&m_tailDescRun)){return false;}
  /*
   if(!i_straightRunner.run(m_tailDescPwm)){return false;}
   if(!i_angularvelocityDetector.setOffsetValue(m_tailDescOffset)){return
   false;} while(!i_angularvelocityDetector.detect()){}
  */
  if (!i_distanceRunner.run(&m_tailDescRun)) {
    return false;
  }
  EV3_LOG("TAIL DESC!");
