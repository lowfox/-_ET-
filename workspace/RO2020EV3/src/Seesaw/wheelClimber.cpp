...
/*
    ...
    if(!i_angleDetector.setTargetValue(m_leftClimbTargetValue)){return false;}
    ...
    //if(!i_angVelDetector.setOffsetValue(m_wheelClimbOffsetValue)){return false;}
    //if(!RyujiEv3Engine::GetRightMotor()->setCounts(10,m_climbPwm,true)){return false;}
    //while(i_angVelDetector.detect()){}
    if(!RyujiEv3Engine::GetLeftMotor()->stop(true)){return false;}
    if(!RyujiEv3Engine::GetRightMotor()->setPWM(m_wheelClimbPwm)){return false;}
    if(!i_tailPositioner.run(&m_climbTail1)){return false;}
