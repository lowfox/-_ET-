...
bool angleDetectorTest::setTargetValueTest(){
    CaseAngleDetector caseAngleTarget[7]={{-181,false},{-180,true},{-1,true},{0,true},{1,true},{180,true},{181,false}};

    angleDetector i_angleDetector;
    for(int i=0;i<7;i++){
        //test of setTargetValue
        if(caseAngleTarget[i].retValue != i_angleDetector.setTargetValue(caseAngleTarget[i].targetValue)){
            //log
            EV3_LOG("TEST_ERR:angleDetector.setTargetValue ...i=%d, targetValue=%d, retVal=%d\n",
...
bool angleDetectorTest::detectTest(){
    ...
    for(int i=0;i<5;i++){
        ...
        if(retDetect[i] != i_angleDetector.detect()){
            EV3_LOG("TEST_ERR:angleDetector.detect ...i=%d, targetValue=%d, caseDetectValue=%d, retVal=%d\n",
