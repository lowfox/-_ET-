...
bool gotoSeesaw::run(void) {
  ...
  */
  ...
  RyujiEv3Engine::GetGyroSensor()->setOffset(0);
  // if(!i_angularVelocityDetector.setOffsetValue(m_seesawDetectValue)){return
  // false;}

  //���C�����A����
  if (!RyujiEv3Engine::GetRightMotor()->setPWM(5)) {
    return false;
  }
  ...
  if (!i_lineTracer.run(m_pwm, m_initAngle)) {
    return false;
  }
  ...
  do {
    m_gotoPeriodDistance =
        gotoNowDistance - gotoLastDistance;  // 660ms����align,last���X�V
    EV3_LOG("GOTO!:get_distance__NowDistance=%f, lastdistance=%f ,",
            gotoNowDistance, gotoLastDistance);
    EV3_LOG("GOTO!:periodDistance=%f\n", m_gotoPeriodDistance);
  } while (m_gotoPeriodDistance > m_detectDistance ||
           m_gotoPeriodDistance < 0);  // 500ms
