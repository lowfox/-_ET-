...
SteeringMode SteeringManager::getMode()
{
	return m_mode;
}
...
bool SteeringManager::stop()
{
	return pSterring->stop();
}

float SteeringManager::driveDistance()
{
...
{
	constexpr float TIRE_DIAMETER = 105.0f;
	constexpr float PI            = 3.1415926535f;

	const float angleLeft       = RyujiEv3Engine::GetLeftMotor()->getCounts()  + m_leftCountOffset;
	const float angleRight      = RyujiEv3Engine::GetRightMotor()->getCounts() + m_rightCountOffset;
	static float prevAngleLeft  = 0.0f;
	static float prevAngleRight = 0.0f;

	const float distance4msLeft  = ((PI * TIRE_DIAMETER) / 360.0f) * (angleLeft  - prevAngleLeft);
	const float distance4msRight = ((PI * TIRE_DIAMETER) / 360.0f) * (angleRight - prevAngleRight);
