...
bool BalanceSterring::update() {
  ...
  balance_control(
      static_cast<float>(m_speed), static_cast<float>(m_turn),
      static_cast<float>(RyujiEv3Engine::GetGyroSensor()->getRate()),
      static_cast<float>(0), static_cast<float>(leftCounts),
      static_cast<float>(rightCounts),
      static_cast<float>(RyujiEv3Engine::GetBattery()->get_mV()), &leftPWM,
      &rightPWM);
